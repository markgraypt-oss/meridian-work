...
async function uploadProfileImageBufferToStorage(
  ...
  await file.save(buffer, {
    ...
    // Persist the PUBLIC ACL the same way getObjectAclPolicy reads it back
  // (file.setMetadata). Setting it inline during file.save did not persist,
  // which left profile images auth-gated: fine on web (session cookie) but
  // blank in the mobile app, whose <Image> sends no auth header.
  await file.setMetadata({ metadata: { 'custom:aclPolicy': JSON.stringify({ owner: 'system', visibility: 'public' }) } });
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/user/preferences', isAuthenticated, async (req: any, res) => {
    try {
      ...
      res.json({
        ...
        dismissedConnectNudge: user.dismissedConnectNudge ?? false,
      });
    } catch (error) {
  ...
  const updatePreferencesSchema = z.object({
    ...
    dismissedConnectNudge: z.boolean().optional(),
  });
  ...
  app.patch('/api/user/preferences', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (parsed.data.dismissedConnectNudge !== undefined) updates.dismissedConnectNudge = parsed.data.dismissedConnectNudge;
      ...
      res.json({
        ...
        dismissedConnectNudge: updatedUser?.dismissedConnectNudge ?? false,
      });
    } catch (error) {
  ...
  app.get('/api/my-programs/:id/stats', isAuthenticated, async (req: any, res) => {
    try {
      ...
      for (const el of exerciseLogs) {
        if (el.section === 'warmup' || el.kind === 'rest') continue;
...
- Interpret the difficulty rating correctly: a low rating means sessions felt easy, a high rating means they were challenging.`;
  ...
  app.patch('/api/programme-workouts/:id/assign-day', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Per-week progression (builder-spec #1): workouts may be assigned to a day in any week.
...
Keep your response concise, practical, and evidence-based. This is general guidance, not medical advice: tell them a suspected deficiency, pregnancy, or any medication needs their doctor. Do not use em dashes. Use plain language.`;
  ...
  const { reportSettings: reportSettingsTable } = await import("@workspace/db");
  ...
  app.post('/api/admin/reports/narrative/feedback', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const { reportNarratives, reportNarrativeFeedback } = await import("@workspace/db");
...
6. One sentence only. No greeting, no sign-off.`;
  ...
  // ============================================
  // Meditations + Smart Recommendations
  // ============================================
  app.get("/api/meditations", isAuthenticated, async (_req: any, res) => {
    try {
      const meds = await storage.getMeditations();
      res.json(meds);
    } catch (err) {
