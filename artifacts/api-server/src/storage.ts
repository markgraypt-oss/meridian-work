...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByIdentifierWithPassword(identifier: string): Promise<User | undefined> {
    const normalized = identifier.trim().toLowerCase();
    const matcher = normalized.includes("@")
      ? sql`lower(${users.email}) = ${normalized}`
      : sql`lower(split_part(${users.email}, '@', 1)) = ${normalized}`;
    const [user] = await db.select().from(users)
      .where(and(matcher, isNotNull(users.password)));
    return user;
  }
  ...
  async getPathContentFromLibrary(pathId: number): Promise<(LearnContentLibraryItem & { orderIndex: number; isRequired: boolean | null; pathContentId: number })[]> {
    const items = await db
      .select({
        ...
        transcript: learnContentLibrary.transcript,
