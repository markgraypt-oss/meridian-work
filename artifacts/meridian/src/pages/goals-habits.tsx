      ...
      <div className="px-6 pt-4 space-y-3">
        {SYSTEM_HABITS.map((habit) => (
          <button
            ...
            className="w-full p-4 bg-card rounded-xl flex items-center gap-4 transition-all shadow-sm hover:shadow-md hover:border-[#0cc9a9] border border-border"
          >
            <div className={`h-11 w-11 rounded-full flex items-center justify-center flex-shrink-0 ${habit.color}`}>
              {habit.icon}
            </div>
            <div className="text-left flex-1 min-w-0">
              <div className="font-semibold text-sm text-foreground">{habit.title}</div>
