...
function PastGoalCard({ goal, calculateProgress, setDeletingGoal, isHighlighted }: { goal: Goal; calculateProgress: (goal: Goal) => number; setDeletingGoal: (goal: Goal) => void; isHighlighted?: boolean }) {
  ...
  return (
    <Card className={`p-4 bg-card transition-all duration-300 ${isHighlighted ? "ring-2 ring-[#0cc9a9] shadow-lg" : ""}`} data-testid={`card-goal-${goal.id}`}>
...
function AddGoalDialogInline({ 
  ...
  const { formatDate } = useFormattedDate();
  const [goalType, setGoalType] = useState<"bodyweight" | "custom" | "nutrition">("bodyweight");
