...
export default function AddProgrammePage() {
  ...
  const { markDirty, handleNavigation, UnsavedChangesDialog } = useUnsavedChanges();
  ...
  const createProgramMutation = useMutation({
    mutationFn: async (data: ProgrammeFormData) => {
      const payload = {
        ...
        duration: data.weeks * 7, // Calculate duration in days
        trainingDaysPerWeek: 0, // Will be updated when workouts are added
    ...
    onSuccess: (newProgram) => {
      ...
      // Set tab to exercises (Workout Content) before navigating
      sessionStorage.setItem('programmeFormTab', 'exercises');
      // Redirect to programme edit page with programmeType for proper back navigation
      navigate(`/admin/programmes/${newProgram.id}${programmeType !== 'main' ? `?programmeType=${programmeType}` : ''}`);
    },
    onError: (error) => {
  ...
  // Get title and context based on programmeType
  const getPageTitle = () => {
    switch (programmeType) {
      case 'stretching': return 'Create Stretching Programme';
      case 'corrective': return 'Create Corrective Exercise Programme';
      default: return 'Create New Programme';
    }
  };
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end space-x-4 pt-4">
              ...
              <Button type="submit" disabled={createProgramMutation.isPending} data-testid="button-save-programme">
