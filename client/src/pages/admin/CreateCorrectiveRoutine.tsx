...
export default function CreateCorrectiveRoutinePage() {
  ...
  const convertLegacyExercisesToBlocks = (exercises: any[]): WorkoutBlock[] => {
    ...
    return exercises.map((exercise, index) => ({
      ...
      exercises: [{
        ...
        sets: exercise.sets || [{ reps: '10 reps', duration: '30 sec' }],
  ...
  useEffect(() => {
    const savedFormData = sessionStorage.getItem('correctiveFormData');
    ...
    if (savedFormData) {
      try {
        ...
        // Ensure routineType is always 'corrective'
        data.routineType = "corrective";
        setFormData(data);
  ...
  useEffect(() => {
    sessionStorage.setItem('correctiveFormData', JSON.stringify(formData));
  }, [formData]);
    ...
    if (selectedId && exercises.length > 0 && blockId) {
      ...
      if (selectedExercise && blockIndex !== -1) {
        const newExercise: BlockExercise = {
          ...
          sets: [{ reps: '10 reps', duration: '30 sec' }],
        };
  ...
  const createRoutineMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Success", description: isEditing ? "Corrective routine updated successfully" : "Corrective routine created successfully" });
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <TopHeader 
        title={isEditing ? "Edit Corrective Routine" : "Create Corrective Routine"}
        onBack={() => {
          sessionStorage.removeItem('correctiveFormData');
          handleNavigation("/admin?tab=corrective");
        }}
      />
          ...
          <Card className="w-full max-w-none">
            <CardContent className="w-full pt-6 space-y-4">
              <div>
                ...
                <input 
                  type="text" 
                  placeholder="e.g., Lower Back Pain Relief, Shoulder Rehab" 
              ...
              <div>
                ...
                <textarea 
                  placeholder="Describe this corrective exercise routine..." 
              ...
              <div className="pt-2">
                ...
                <div className="space-y-2">
                  {([
                    { value: 'regular' as const, label: 'Regular', desc: 'Standard routine with sets and reps' },
