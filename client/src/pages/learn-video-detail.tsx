...
import type { LearningPath, UserFavourite, LearnContentLibraryItem } from "@shared/schema";

type PathLibraryContent = LearnContentLibraryItem & { orderIndex: number; isRequired: boolean | null; pathContentId: number };

export default function LearnVideoDetail() {
  const [, params] = useRoute("/learn/path/:pathId/video/:contentId");
  const pathId = parseInt(params?.pathId || "0");
  ...
  const { data: contentItems = [] } = useQuery<PathLibraryContent[]>({
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="px-4 pt-4">
        ...
        <p className="text-sm text-muted-foreground mb-3" data-testid="text-lesson-progress">
          Lesson {currentIndex + 1} of {sortedItems.length}
        </p>
        ...
        <div className="flex gap-2 mb-6">
          <Button
            variant="outline"
            size="icon"
            className={`h-10 w-10 rounded-full ${isCompleted ? 'bg-green-500 border-green-500 text-white' : ''}`}
        ...
        {nextItem && (
          <div className="mb-6">
            ...
            <Card 
              className="overflow-hidden cursor-pointer hover:shadow-md transition-shadow"
              onClick={() => navigate(`/learn/path/${pathId}/video/${nextItem.id}`)}
