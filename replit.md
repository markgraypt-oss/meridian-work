...
- **Post-Workout Summary Flow**: Finishing a workout in `active-workout` no longer shows a separate celebration overlay. Instead the page caches `{summary, prs}` in `sessionStorage` under `workout-completion-${id}` and navigates to `/progress/workout/${id}?justCompleted=1`. The new `workout-log-view` renders a hero (cover image or category-typed gradient), a PR banner (real PRs only, excluding `type==='first'`), 3 stats (length, volume, effort pill green/teal/red by 1-3/4-6/7-10), an AI review card backed by `POST /api/workout-logs/:id/ai-review` (cached on `workout_logs.aiReviewText`/`aiReviewedAt`, hyphen-not-em-dash enforced both in prompt and via post-call `\u2014` strip, 120-180 word plain prose), the existing WorkoutCompletionView, a personal-bests detail card, and bottom CTAs on `justCompleted` only. "Continue in coach chat" dispatches a window `open-coach` event listened to in `App.tsx` and writes a rich `coach-seed-context` JSON `{summaryText, openingMessage}` that `CoachChat` consumes on open: it shows the opening assistant message, suppresses the proactive greeting, and prepends the summary text invisibly to the first user turn so the AI has full session context. `POST /api/workout-logs/:id/complete` performs an ownership check (IDOR protection) and persists the rating-dialog `notes` field before completion so the AI review can use them.

- **Wearables Integration**: OAuth-based (Oura, WHOOP, Google Fit) plus upload-based Apple Health export.zip. Schema in `shared/schema.ts`: `wearable_connections` (encrypted access/refresh tokens via AES-256-GCM in `server/wearables/encryption.ts`, status `connected|disconnected|needs_reauth`), `wearable_metrics_daily` (unique on user+date+provider; sleep stages, HRV, RHR, steps, active minutes, readiness/strain), `wearable_sync_logs`. Adapters in `server/wearables/{oura,whoop,googleFit,appleHealth}.ts` with shared orchestrator `index.ts` (`syncProvider`, `upsertDailyMetrics` with `onConflictDoUpdate`, `refreshIfNeeded` for token rotation, `getRecentWearableMetrics` returning best-priority provider per date: oura > whoop > apple_health > google_fit). Routes in `server/wearables/routes.ts`: `GET /api/wearables/{providers,connections,today}`, `GET /api/wearables/connect/:provider` (returns auth URL — opened in popup), `GET /api/wearables/callback/:provider` (exchanges code, fires initial sync, postMessage to opener), `POST /api/wearables/{disconnect,sync}/:provider`, `POST /api/wearables/apple-health/upload` (multer 200MB, parses export.xml from zip via `adm-zip`). Hourly scheduler in `server/wearables/scheduler.ts` (started from `server/index.ts`) re-syncs any connection stale > 6h. Burnout engine (`server/burnoutEngine.ts`) accepts optional `wearableMetrics` and merges them in front of self-reported sleep/step entries (per-date dedup so partial wearable coverage falls back to manual). AI context (`server/aiProvider.ts:getUserDataContext`) prefers wearable rows for sleep/steps/resting_hr domains and tags each section "from <Device Label>" so coaches cite the source. Frontend `client/src/pages/integrations.tsx` lists providers with status badges, sync/disconnect buttons, connect-via-popup with postMessage handshake, and Apple Health file upload. Required env vars (per provider): `OURA_CLIENT_ID/SECRET`, `WHOOP_CLIENT_ID/SECRET`, `GOOGLE_FIT_CLIENT_ID/SECRET`; token encryption uses `WEARABLE_TOKEN_KEY` (falls back to `SESSION_SECRET`). Apple Health works without any env vars (upload-only).
