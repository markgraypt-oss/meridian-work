...
export const users = pgTable("users", {
  ...
  intervalVoiceCuesEnabled: boolean("interval_voice_cues_enabled").notNull().default(true),
  intervalAnnounceExerciseNames: boolean("interval_announce_exercise_names").notNull().default(true),
...
export const notificationPreferences = pgTable("notification_preferences", {
  ...
  supplementMorningEnabled: boolean("supplement_morning_enabled").default(true),
  supplementMorningTime: varchar("supplement_morning_time").default("08:00"),
  supplementAfternoonEnabled: boolean("supplement_afternoon_enabled").default(true),
  supplementAfternoonTime: varchar("supplement_afternoon_time").default("13:00"),
  supplementEveningEnabled: boolean("supplement_evening_enabled").default(true),
  supplementEveningTime: varchar("supplement_evening_time").default("20:00"),
...
export const programs = pgTable("programs", {
  ...
  // AI provenance: 'manual' (default) or 'ai' (generated by Programme Generator)
...
export const workouts = pgTable("workouts", {
  ...
  // AI provenance: 'manual' (default) or 'ai' (generated by Workout Generator)
  source: text("source").notNull().default("manual"),
  generationInputs: jsonb("generation_inputs"), // raw inputs used by AI generator
  createdAt: timestamp("created_at").defaultNow(),
});
...
// ====================================================================
// BURNOUT CALIBRATION EVENTS
// ====================================================================
// The evidence foundation for the Early Warning system. Revolutionary
// requirement #1 demands we PROVE physiological warnings precede burnout
// trajectory changes more often than chance. We cannot prove that without
// recording every prediction and later checking what actually happened.
//
// Each row is written FORWARD at burnout computation time (we capture the
// physiological state, whether a warning fired, and the trajectory at that
// moment). The outcome fields are filled BACKWARD by a later analysis job
// once enough days have passed to observe whether the trajectory worsened.
//
// This table is intentionally write-now / validate-later. It accumulates
// silently for weeks before it has enough paired (warning -> outcome) data
// to compute predictive validity. Until then it surfaces nothing to users.
export const burnoutCalibrationEvents = pgTable("burnout_calibration_events", {
  id: serial("id").primaryKey(),
  userId: varchar("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),

  // --- Written FORWARD (at computation time) ---

  // When this calibration snapshot was taken
  computedAt: timestamp("computed_at").notNull().defaultNow(),

  // Physiological deviations from personal baseline at this moment, expressed
  // in standard deviations (z-scores). Negative HRV z = HRV below baseline
  // (bad). Positive RHR z = RHR above baseline (bad). Negative sleep z = less
  // sleep than baseline (bad). Null when the metric was unavailable.
  hrvZScore: real("hrv_z_score"),
  rhrZScore: real("rhr_z_score"),
  sleepZScore: real("sleep_z_score"),

  // Whether the baseline was calibrated at this point. If false, the z-scores
  ...
  // Did an early-warning condition fire at this computation? And which ones?
  // warningFlags is an array of short codes e.g. ["hrv_suppressed_3d", "rhr_elevated"].
  warningFired: boolean("warning_fired").notNull().default(false),
  warningFlags: jsonb("warning_flags").notNull().default(sql`'[]'::jsonb`),

  // The burnout state AT THIS MOMENT (the "before" picture).
  scoreAtEvent: integer("score_at_event"),
  trajectoryAtEvent: text("trajectory_at_event"), // stable | rising | elevated | recovering

  // --- Filled BACKWARD (by the analysis job, days later) ---

  // Set true once the analysis job has processed this row's outcome window.
  outcomeEvaluated: boolean("outcome_evaluated").notNull().default(false),
  outcomeEvaluatedAt: timestamp("outcome_evaluated_at"),

  // Did the trajectory worsen within the outcome window after this event?
  // (e.g. stable->rising, rising->elevated). Null until evaluated.
  trajectoryWorsened: boolean("trajectory_worsened"),

  // The worst trajectory observed in the outcome window, and the score delta
  // (max score in window minus scoreAtEvent). Lets us measure magnitude, not
  // just direction. Null until evaluated.
  worstTrajectoryInWindow: text("worst_trajectory_in_window"),
  scoreDeltaInWindow: integer("score_delta_in_window"),

  // How many days the outcome window covered (e.g. 7). Stored so we can vary
  // the window later without losing the meaning of historical rows.
  outcomeWindowDays: integer("outcome_window_days"),

  createdAt: timestamp("created_at").defaultNow(),
}, (t) => ({
  userComputedIdx: index("burnout_calibration_events_user_computed_idx").on(t.userId, t.computedAt),
  unevaluatedIdx: index("burnout_calibration_events_unevaluated_idx").on(t.outcomeEvaluated),
}));
...
// Programme/workout AI generations - audit trail for the AI Programme & Workout Generator.
// One row per accepted (saved) generation; aiCallLogs already records the underlying call.
export const programGenerations = pgTable("program_generations", {
  id: serial("id").primaryKey(),
  userId: varchar("user_id"), // requesting user (admin or end user)
  kind: text("kind").notNull(), // 'programme' | 'workout'
  inputs: jsonb("inputs").notNull(), // structured inputs used to generate
  rawResponse: jsonb("raw_response"), // parsed JSON returned by the model
  provider: text("provider"),
  model: text("model"),
  programId: integer("program_id"), // set when kind='programme' and saved
  workoutId: integer("workout_id"), // set when kind='workout' and saved
  aiCallLogId: integer("ai_call_log_id"), // FK-ish to aiCallLogs.id
  createdAt: timestamp("created_at").defaultNow(),
}, (table) => [
  index("idx_program_generations_user").on(table.userId),
  index("idx_program_generations_created_at").on(table.createdAt),
]);
